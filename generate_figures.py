...
# My datasets
url_bikini      = 'https://drive.google.com/open?id=1uckf1_tXIn1rQW0f6H_wScYMAjCiddyo' #network-snapshot-007440.pkl
#/dataset/results/00000-sgan-custom-images-1gpu/
url_bikini_local = "/dataset/results/00000-sgan-custom-images-1gpu/network-snapshot-007440.pkl" #network-snapshot-007440.pkl
...
def main():
    ...
    # draw_uncurated_result_figure(os.path.join(config.result_dir, 'figure02-uncurated-ffhq.png'), load_Gs(url_bikini_local), cx=0, cy=0, cw=1024, ch=1024, rows=3, lods=[0,1,2,2,3,3], seed=5)
    draw_style_mixing_figure(os.path.join(config.result_dir, 'figure03-style-mixing1.png'), load_Gs(url_bikini_local), w=512, h=512, src_seeds=[639,701,687,615,2268], dst_seeds=[888,829,1898,1733,1614,845], style_ranges=[range(0,2)]*3+[range(2,4)]*2+[range(4,9)])
    draw_style_mixing_figure(os.path.join(config.result_dir, 'figure03-style-mixing2.png'), load_Gs(url_bikini_local), w=512, h=512, src_seeds=[639,701,687,615,2268], dst_seeds=[888,829,1898,1733,1614,845], style_ranges=[range(0,2)]*3+[range(2,4)]*2+[range(4,9)])


    # draw_style_mixing_figure(os.path.join(config.result_dir, 'figure03-style-mixing.png'), load_Gs(url_bikini_local), w=512, h=512, src_seeds=[639,701,687,615,2268], dst_seeds=[888,829,1898,1733,1614,845], style_ranges=[range(0,4)]*3+[range(4,8)]*2+[range(8,18)])

    draw_noise_detail_figure(os.path.join(config.result_dir, 'figure04-noise-detail.png'), load_Gs(url_bikini_local), w=512, h=512, num_samples=100, seeds=[1157,1012])
    draw_noise_components_figure(os.path.join(config.result_dir, 'figure05-noise-components.png'), load_Gs(url_bikini_local), w=512, h=512, seeds=[1967,1555], noise_ranges=[range(0, 18), range(0, 0), range(8, 18), range(0, 8)], flips=[1])
    draw_style_mixing_figure(os.path.join(config.result_dir, 'figure03-style-mixing3.png'), load_Gs(url_bikini_local), w=512, h=512, src_seeds=[639,701,687,615,2268], dst_seeds=[888,829,1898,1733,1614,845], style_ranges=[range(0,2)]*3+[range(3,5)]*2+[range(5,10)])

    # draw_truncation_trick_figure(os.path.join(config.result_dir, 'figure08-truncation-trick.png'), load_Gs(url_bikini_local), w=1024, h=1024, seeds=[91,388], psis=[1, 0.7, 0.5, 0, -0.5, -1])
    ...
    # draw_uncurated_result_figure(os.path.join(config.result_dir, 'figure11-uncurated-cars.png'), load_Gs(url_bikini_local), cx=0, cy=64, cw=512, ch=384, rows=4, lods=[0,1,2,2,3,3], seed=2)
...
if __name__ == "__main__":
    main()
    #floyd run --gpu --env pytorch-1.0 --data wmreynol5/datasets/dataset/3:dataset "python3 generate_figures.py"
